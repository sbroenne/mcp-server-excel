...
- Advertised counts are generated by `scripts\check-doc-counts.ps1 -Update`, run
  by `.github/workflows/doc-counts.yml` on every push to `main`, not counted
  from tool files or CLI folders. That workflow writes the single generated
  include file `doc-counts.json` (repo root); every other consumer (release
  automation, `gh-pages/hooks.py`) reads that file instead of re-deriving or
  parsing headline text. Internal CLI diagnostics are excluded from the shared
  advertised surface.
