...
**NEVER `--no-verify` (Rule 31):** Never bypass pre-commit hooks with `--no-verify`/`-n`/`HUSKY=0`/`core.hooksPath`. Let all 14 gates run to completion. The `CI Gate` workflow (`.github/workflows/ci.yml`) enforces the **Excel-free** gates on every PR (Release build, COM-leak, success-flag, coverage/naming, MCP-Core, doc-counts, dynamic-cast, plugin-README) on the GitHub-hosted `windows-latest` runner, which has no Excel. The **Excel-dependent** gates (CLI workflow smoke, MCP smoke) run in `.github/workflows/integration-tests.yml` on the self-hosted `[self-hosted, windows, excel]` runner, alongside every PR's Excel integration test suite — so they ARE verified server-side too, not just in pre-commit. The packaging deliverable gates still run **only** in pre-commit — so bypassing still ships those unverified. If a hook fails or hangs (incl. environment issues like VBA trust disabled), STOP and ask the user — never bypass.
...
| 7 | CLI Workflow Test | `Test-CliWorkflow.ps1` | E2E CLI workflow smoke test (also runs in `integration-tests.yml` on the self-hosted Excel runner) |
| 8 | MCP Smoke Test | `dotnet test --filter "...SmokeTest..."` | All MCP tools functional (also runs in `integration-tests.yml` on the self-hosted Excel runner) |
...
**PR/CI enforcement on this repo (single-maintainer):** `sbroenne` is the **sole developer**, so the `main` ruleset requires **0 human approvals** (GitHub forbids approving your own PR — a required-approval rule would deadlock every merge). The effective review gate is the **mandatory `copilot_code_review` rule**. Required status check: **`CI Gate`** (`.github/workflows/ci.yml`), which runs only the **Excel-free** gates on the GitHub-hosted `windows-latest` runner (no Excel installed). The Excel-dependent gates (CLI workflow smoke, MCP smoke) plus the full Excel integration test suite run separately in `.github/workflows/integration-tests.yml`, on a self-hosted `[self-hosted, windows, excel]` Azure VM runner that does have Excel installed — that workflow is not currently a required status check, but it runs automatically on every non-draft PR. Merges are squash-only with `delete_branch_on_merge: true` and auto-merge enabled.
