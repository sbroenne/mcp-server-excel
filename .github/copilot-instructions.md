...
**NEVER `--no-verify` (Rule 31):** Never bypass pre-commit hooks with `--no-verify`/`-n`/`HUSKY=0`/`core.hooksPath`. Let every applicable gate selected by the hook run to completion. The `CI Gate` workflow (`.github/workflows/ci.yml`) enforces the **Excel-free** gates on every PR (Release build, COM-leak, success-flag, coverage/naming, MCP-Core, doc-counts, dynamic-cast, plugin-README) on the GitHub-hosted `windows-latest` runner, which has no Excel. Run the **Excel-dependent** CLI workflow and MCP smoke gates locally with `& .\scripts\Test-E2E.ps1` only when staged changes affect the Core, CLI, or MCP runtime path; this includes `ComInterop`, `Service`, and source-generator changes that feed those surfaces. Plugin/build/publish, marketplace, skill, documentation, and test-only changes do not require Excel E2E unless the same commit also changes a runtime path. The packaging deliverable gates also run **only** in pre-commit — so bypassing ships unverified code. If an applicable hook fails or hangs (incl. environment issues like VBA trust disabled), STOP and ask the user — never bypass.
...
| 6b | Documentation Count Validation | `check-doc-counts.ps1` | All user-facing docs report the code-derived tool/operation counts (31 tools / 326 operations) |
| 7 | CLI Workflow Test | `Test-E2E.ps1` | Conditional local E2E CLI smoke test when staged changes affect Core, CLI, or MCP runtime paths |
| 8 | MCP Smoke Test | `Test-E2E.ps1` | Conditional local MCP all-tools validation for the same runtime-impacting changes |
...
**PR/CI enforcement on this repo (single-maintainer):** `sbroenne` is the **sole developer**, so the `main` ruleset requires **0 human approvals** (GitHub forbids approving your own PR — a required-approval rule would deadlock every merge). The effective review gate is the **mandatory `copilot_code_review` rule**. Required status check: **`CI Gate`** (`.github/workflows/ci.yml`), which runs only the **Excel-free** gates on the GitHub-hosted `windows-latest` runner (no Excel installed). For changes that affect Core, CLI, or MCP runtime paths, run feature-specific Excel integration tests locally with the appropriate `dotnet test ... --filter "Feature=<name>&RunType!=OnDemand"` command and run `& .\scripts\Test-E2E.ps1`; this scope includes `ComInterop`, `Service`, and relevant generator changes. Do not require Excel E2E for plugin/build/publish, marketplace, skill, documentation, or test-only changes unless they share a commit with runtime-path changes. Check the applicable PR attestations only after the required commands complete without failures or unresolved issues. Merges are squash-only with `delete_branch_on_merge: true` and auto-merge enabled.
