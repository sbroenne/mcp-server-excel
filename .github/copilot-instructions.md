...
### Excel Instance Pooling Pattern (MCP Server Optimization)

**Problem:** Starting and closing Excel for every command has ~2-5 second overhead.

**Solution:** `ExcelInstancePool` reuses Excel instances across multiple operations on the same workbook.

**Architecture:**
```
ExcelInstancePool (Core)
    ↓
ExcelHelper.InstancePool (static property)
    ↓
ExcelHelper.WithExcel() (auto-detects pool)
    ↓
All Core Commands (zero changes needed)
```
...
**CLI Behavior:**
- ✅ **No pooling** - CLI continues to use single-instance pattern
- ✅ **Simple & reliable** - Each command is isolated
- ✅ **Backward compatible** - No breaking changes

**When to Use Pooling:**
- ✅ **MCP Server** - Always (configured automatically)
- ✅ **Batch operations** - Multiple commands on same workbook
...
// Use normally - pooling is automatic
ExcelHelper.WithExcel(filePath, save, (excel, workbook) => { ... });
...
**Advanced Operations:**
```csharp
// Close workbook but keep Excel instance pooled
pool.CloseWorkbook(filePath);
