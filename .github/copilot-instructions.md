...
| 6b | Documentation Count Validation | `check-doc-counts.ps1` | MCP docs report the code-derived 29 tools / 246 operations; CLI docs report 18 groups / 243 operations |
| 7 | CLI Workflow Test | `Test-E2E.ps1` | Local E2E CLI workflow smoke test |
| 8 | MCP Smoke Test | `Test-E2E.ps1` | Local E2E validation that all MCP tools are functional |
...
**PR/CI enforcement on this repo (single-maintainer):** `sbroenne` is the **sole developer**, so the `main` ruleset requires **0 human approvals** (GitHub forbids approving your own PR — a required-approval rule would deadlock every merge). The effective review gate is the **mandatory `copilot_code_review` rule**. Required status check: **`CI Gate`** (`.github/workflows/ci.yml`), which runs only the **Excel-free** gates on the GitHub-hosted `windows-latest` runner (no Excel installed). Run feature-specific Excel integration tests locally with the appropriate `dotnet test ... --filter "Feature=<name>&RunType!=OnDemand"` command. Separately, run the Excel-dependent CLI and MCP E2E smoke gates with `& .\scripts\Test-E2E.ps1`; check the PR attestations only after both complete without failures or unresolved issues. Merges are squash-only with `delete_branch_on_merge: true` and auto-merge enabled.
