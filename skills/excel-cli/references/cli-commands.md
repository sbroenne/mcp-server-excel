...
Conditional formatting - visual rules based on cell values. TYPES: cellValue, expression, colorScale, dataBar, iconSet, top10, aboveAverage, timePeriod, uniqueValues, blanksCondition. Basic rules use operator/formula parameters (cellValue requires operatorType and formula1; expression requires formula1); visual rules use dedicated parameters and return their type-specific configuration from list-rules / list-worksheet-rules. Both camelCase and kebab-case values are accepted. FORMAT: interiorColor/fontColor as #RRGGBB, fontBold/Italic, borderStyle/Color. OPERATORS: equal, notEqual, greater, less, greaterEqual, lessEqual, between, notBetween. For 'between' and 'notBetween', both formula1 and formula2 are required.
