...
**Full reference**: See [cli-commands.md](./references/cli-commands.md) or run `excelcli <command> --help`.

Available commands: `calculationmode`, `chart`, `chartconfig`, `conditionalformat`, `connection`, `datamodel`, `datamodelrelationship`, `diag`, `namedrange`, `pivottable`, `pivottablecalc`, `pivottablefield`, `powerquery`, `range`, `rangeedit`, `rangeformat`, `rangelink`, `screenshot`, `sheet`, `worksheetstyle`, `slicer`, `table`, `tablecolumn`, `vba`, `window`.

## Common Pitfalls

See [cli-commands.md](./references/cli-commands.md#common-pitfalls) for detailed pitfall examples. Key issues:

- **`--values-file`** expects a path to an existing file — do not pass inline JSON
- **`--timeout`** must be a positive integer; `--timeout 0` is invalid
- **`--values`** takes a 2D JSON array: `'[["Name","Age"],["Alice",30]]'`
- **List parameters** (e.g., `--selected-items`) require JSON arrays: `'["West","East"]'`
- **Power Query** operations may take 30+ seconds; use generous timeouts
