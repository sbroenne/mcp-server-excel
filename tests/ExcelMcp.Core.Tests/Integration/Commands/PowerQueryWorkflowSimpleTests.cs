using Sbroenne.ExcelMcp.Core.Commands;
using Sbroenne.ExcelMcp.ComInterop.Session;
using Xunit;

namespace Sbroenne.ExcelMcp.Core.Tests.Integration.Commands;

/// <summary>
/// Simple integration tests for Power Query workflow guidance using batch pattern
/// </summary>
[Trait("Category", "Integration")]
[Trait("Speed", "Fast")]
[Trait("Layer", "Core")]
[Trait("Feature", "PowerQuery")]
[Trait("RequiresExcel", "true")]
public class PowerQueryWorkflowSimpleTests : IDisposable
{
    private readonly string _testDir;
    private readonly string _testFile;
    private readonly PowerQueryCommands _commands;
    private readonly FileCommands _fileCommands;

    public PowerQueryWorkflowSimpleTests()
    {
        _testDir = Path.Combine(Path.GetTempPath(), $"ExcelMcp_PQWorkflow_{Guid.NewGuid():N}");
        Directory.CreateDirectory(_testDir);
        _testFile = Path.Combine(_testDir, "test.xlsx");
        _commands = new PowerQueryCommands();
        _fileCommands = new FileCommands();

        // Create test workbook
        var result = _fileCommands.CreateEmptyAsync(_testFile, overwriteIfExists: false).GetAwaiter().GetResult();
        if (!result.Success)
        {
            throw new InvalidOperationException($"Failed to create test file: {result.ErrorMessage}");
        }
    }

    public void Dispose()
    {
        try
        {
            if (Directory.Exists(_testDir))
            {
                Directory.Delete(_testDir, recursive: true);
            }
        }
        catch { /* Cleanup failure is non-critical */ }
        GC.SuppressFinalize(this);
    }

    [Fact]
    public async Task Import_BasicQuery_ShowsWorkflowHints()
    {
        // Arrange
        const string queryName = "TestQuery";
        var mCodeFile = Path.Combine(_testDir, $"{queryName}.m");
        File.WriteAllText(mCodeFile, @"let
    Source = Excel.CurrentWorkbook(){[Name=""Sheet1""]}[Content]
in
    Source");

        // Act
        await using var batch = await ExcelSession.BeginBatchAsync(_testFile);
        var result = await _commands.ImportAsync(batch, queryName, mCodeFile);

        // Assert
        Assert.True(result.Success, $"Import failed: {result.ErrorMessage}");
        // Workflow hints are included in the result for guidance
    }

    [Fact]
    public async Task List_EmptyWorkbook_ReturnsSuccess()
    {
        // Act
        await using var batch = await ExcelSession.BeginBatchAsync(_testFile);
        var result = await _commands.ListAsync(batch);

        // Assert
        Assert.True(result.Success, $"List failed: {result.ErrorMessage}");
        Assert.NotNull(result.Queries);
        Assert.Empty(result.Queries);
    }
}
