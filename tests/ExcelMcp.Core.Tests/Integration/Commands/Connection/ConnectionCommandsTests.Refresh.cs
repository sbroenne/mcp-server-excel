...
public partial class ConnectionCommandsTests
{
    ...
    public async Task RefreshAsync_ConnectionNotFound_ReturnsFailure()
    {
        ...
        var testFile = await CoreTestHelper.CreateUniqueTestFileAsync(
            ...
            nameof(RefreshAsync_ConnectionNotFound_ReturnsFailure),
            _tempDir);

        // Act
        await using var batch = await ExcelSession.BeginBatchAsync(testFile);
        var result = await _commands.RefreshAsync(batch, "NonExistentConnection");
    ...
    public async Task RefreshAsync_ConnectionOnlyQuery_ReturnsSuccessWithContext()
    {
        ...
        var testFile = await CoreTestHelper.CreateUniqueTestFileAsync(
            nameof(ConnectionCommandsTests),
            nameof(RefreshAsync_ConnectionOnlyQuery_ReturnsSuccessWithContext),
            _tempDir);
        ...
        var result = await _commands.RefreshAsync(batch, connectionName);
    ...
    public async Task RefreshAsync_ConnectionWithLoadedData_ReturnsSuccess()
    {
        ...
        var testFile = await CoreTestHelper.CreateUniqueTestFileAsync(
            nameof(ConnectionCommandsTests),
            nameof(RefreshAsync_ConnectionWithLoadedData_ReturnsSuccess),
            _tempDir);
        ...
        await using var batch = await ExcelSession.BeginBatchAsync(testFile);
        await _commands.LoadToAsync(batch, connectionName, "TestSheet");
    ...
    public async Task RefreshAsync_WithTimeout_RespectsTimeoutParameter()
    {
        ...
        var testFile = await CoreTestHelper.CreateUniqueTestFileAsync(
            nameof(ConnectionCommandsTests),
            nameof(RefreshAsync_WithTimeout_RespectsTimeoutParameter),
            _tempDir);
        ...
        var result = await _commands.RefreshAsync(batch, connectionName, customTimeout);
    ...
    public async Task RefreshAsync_TextConnectionMissingFile_SucceedsWithoutValidation()
    {
        ...
        var testFile = await CoreTestHelper.CreateUniqueTestFileAsync(
            nameof(ConnectionCommandsTests),
            nameof(RefreshAsync_TextConnectionMissingFile_SucceedsWithoutValidation),
            _tempDir);
