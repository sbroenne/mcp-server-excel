...
The CLI provides 18 command categories with 243 operations shared with the MCP Server. The MCP surface adds handwritten file/session and workflow tools, for 29 tools and 246 operations in total. The CLI uses **64% fewer tokens** than the traditional MCP tool surface because it wraps its operations in a single tool with skill-based guidance instead of loading every tool schema into context.
...
ExcelMcp.CLI provides **243 operations** across 18 command categories: Power Query, Data Model/DAX, PivotTables, Excel Tables, Charts, VBA, Ranges, Worksheets, Connections, Named Ranges, Conditional Formatting, Slicers, Calculation Mode, Python in Excel, Screenshot, File & Session, and Window Management.
