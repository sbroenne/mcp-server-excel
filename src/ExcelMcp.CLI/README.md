...
The CLI provides 18 command categories with 262 operations matching the MCP Server — the same capabilities, just packaged as 18 CLI command categories instead of the MCP Server's 27 tools. It uses **64% fewer tokens** than the MCP Server because it wraps all operations in a single tool with skill-based guidance instead of loading 27 tool schemas into context.
...
ExcelMcp.CLI provides **262 operations** across 18 command categories: Power Query, Data Model/DAX, PivotTables, Excel Tables, Charts, VBA, Ranges, Worksheets, Workbook protection, Connections, Named Ranges, Conditional Formatting, Slicers, Calculation Mode, Python in Excel, Screenshot, File & Session, and Window Management.
