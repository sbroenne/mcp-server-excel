...
public class ServiceRegistryGenerator : IIncrementalGenerator
{
    ...
    private static void Execute(Compilation compilation, ImmutableArray<InterfaceDeclarationSyntax?> interfaces, SourceProductionContext context)
    {
        ...
        if (allCategories.Count > 0)
        {
            ...
            var safetyManifestCode = GenerateSafetyManifest(allCategories);
            context.AddSource("ServiceRegistry.Safety.g.cs", SourceText.From(safetyManifestCode, Encoding.UTF8));
    ...
    private static string GenerateSafetyManifest(IReadOnlyList<ServiceInfo> categories)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated />");
        sb.AppendLine("#nullable enable");
        sb.AppendLine("#pragma warning disable CS1591 // Missing XML comment for generated public members");
        sb.AppendLine("namespace Sbroenne.ExcelMcp.Generated;");
        sb.AppendLine();
        sb.AppendLine("public static partial class ServiceRegistry");
        sb.AppendLine("{");
        sb.AppendLine("    public static readonly System.Collections.Generic.IReadOnlyDictionary<string, Sbroenne.ExcelMcp.Core.Models.CommandSafetyDescriptor> SafetyDescriptors =");
        sb.AppendLine("        new System.Collections.Generic.Dictionary<string, Sbroenne.ExcelMcp.Core.Models.CommandSafetyDescriptor>(System.StringComparer.OrdinalIgnoreCase)");
        sb.AppendLine("        {");

        foreach (var category in categories.OrderBy(category => category.Category, StringComparer.Ordinal))
        {
            foreach (var method in category.Methods.OrderBy(method => method.ActionName, StringComparer.Ordinal))
            {
                var command = $"{category.Category}.{method.ActionName}";
                var mutationKind = DetermineMutationKind(category, method);
                var scopeResolver = DetermineScopeResolver(method);
                var verificationLevel = DetermineVerificationLevel(category, method, scopeResolver);
                var checkpointRecommended = method.IsMutation && category.Category is not "window" and not "calculation";
                var recoveryRisk = DetermineRecoveryRisk(category, method);

                sb.AppendLine($"            [\"{command}\"] = new(\"{command}\", {(method.IsMutation ? "true" : "false")}, \"{mutationKind}\", \"{scopeResolver}\", \"{verificationLevel}\", {(checkpointRecommended ? "true" : "false")}, \"{recoveryRisk}\", true),");
            }
        }

        sb.AppendLine("        };");
        sb.AppendLine();
        sb.AppendLine("    public static Sbroenne.ExcelMcp.Core.Models.CommandSafetyDescriptor GetSafetyDescriptor(string command) =>");
        sb.AppendLine("        SafetyDescriptors.TryGetValue(command, out var descriptor)");
        sb.AppendLine("            ? descriptor");
        sb.AppendLine("            : Sbroenne.ExcelMcp.Core.Models.CommandSafetyDescriptor.Unknown(command);");
        sb.AppendLine("}");
        return sb.ToString();
    }

    private static string DetermineMutationKind(ServiceInfo category, MethodInfo method)
    {
        ...
        if (category.Category == "vba" && action == "run") return "macroExecution";
        if (category.Category is "powerquery" or "connection" || action.Contains("refresh", StringComparison.Ordinal) || action == "calculate") return "externalRefresh";
        if (category.Category is "datamodel" or "datamodelrel" or "pivottable" or "pivottablefield" or "pivottablecalc") return "modelStructure";
        if (category.Category is "window" or "calculation") return "settings";
        if (category.Category == "pythoninexcel" || action.Contains("formula", StringComparison.Ordinal)) return "formulas";
        if (category.Category is "rangeformat" or "sheetstyle" or "conditionalformat" ||
            action.Contains("format", StringComparison.Ordinal) || action.Contains("style", StringComparison.Ordinal) ||
            action.Contains("width", StringComparison.Ordinal) || action.Contains("height", StringComparison.Ordinal) ||
            action.Contains("merge", StringComparison.Ordinal) || action.Contains("validation", StringComparison.Ordinal) ||
            action.Contains("hyperlink", StringComparison.Ordinal) || action.Contains("lock", StringComparison.Ordinal)) return "formatting";
        if (category.Category is "range" or "rangeedit" or "rangelink") return "values";
        return "workbookStructure";
    }

    private static string DetermineScopeResolver(MethodInfo method)
    {
        var parameterNames = method.Parameters
            .Select(parameter => parameter.Name)
            .ToArray();
        if (parameterNames.Contains("sourceSheet", StringComparer.Ordinal) &&
            parameterNames.Contains("sourceRange", StringComparer.Ordinal) &&
            parameterNames.Contains("targetSheet", StringComparer.Ordinal) &&
            parameterNames.Contains("targetRange", StringComparer.Ordinal)) return "rangeCopy";
        if (method.Parameters.Any(parameter => parameter.Name == "rangeAddress")) return "range";
        if (method.Parameters.Any(parameter => parameter.Name == "sheetName")) return "worksheet";
        if (method.Parameters.Any(parameter => parameter.Name == "tableName")) return "table";
        if (method.Parameters.Any(parameter => parameter.Name == "chartName")) return "chart";
        if (method.Parameters.Any(parameter => parameter.Name == "pivotTableName")) return "pivotTable";
        if (method.Parameters.Any(parameter => parameter.Name is "queryName" or "connectionName")) return "externalObject";
        return "workbook";
    }

    private static string DetermineVerificationLevel(ServiceInfo category, MethodInfo method, string scopeResolver)
    {
        if (!method.IsMutation) return "none";
        if (scopeResolver is "range" or "rangeCopy")
        {
            // The range inspector captures values and formulas, but not formatting,
            // validation, hyperlinks, or other range metadata. Only value/formula
            // mutations have enough evidence to claim exact range verification.
            var mutationKind = DetermineMutationKind(category, method);
            if (scopeResolver == "rangeCopy" && method.ActionName == "copy")
            {
                return "rangeFingerprint";
            }
            return mutationKind is "values" or "formulas"
                ? "rangeSemantic"
                : "rangeFingerprint";
        }
        if (scopeResolver is "worksheet" or "table" or "chart" or "pivotTable") return "collectionSemantic";
        if (category.Category is "window" or "calculation") return "settingSemantic";
        if (category.Category == "vba" && method.ActionName == "run") return "notVerified";
        return "workbookFingerprint";
    }

    private static string DetermineRecoveryRisk(ServiceInfo category, MethodInfo method)
    {
        if (!method.IsMutation) return "none";
        if (category.Category is "vba" or "powerquery" or "connection" or "datamodel" or "datamodelrel" || method.ActionName.Contains("refresh", StringComparison.Ordinal)) return "high";
        return "medium";
    }
    ...
    private static string GenerateServiceRegistry(ServiceInfo info)
    {
        ...
        sb.AppendLine("        /// <summary>");
        sb.AppendLine("        /// Returns whether the supplied action requires an existing Excel session.");
        sb.AppendLine("        /// </summary>");
        sb.AppendLine($"        public static bool RequiresSessionForAction({info.CategoryPascal}Action action) => action switch");
        sb.AppendLine("        {");
        foreach (var method in info.Methods)
        {
            sb.AppendLine($"            {info.CategoryPascal}Action.{method.MethodName} => {(method.RequiresSession ? "true" : "false")},");
        }
        sb.AppendLine("            _ => RequiresSession");
        sb.AppendLine("        };");
        sb.AppendLine();
    ...
    private static void GenerateCliSettings(StringBuilder sb, ServiceInfo info, List<ExposedParameter> allParams)
    {
        ...
        if (!info.NoSession)
        {
            ...
            sb.AppendLine("            [Spectre.Console.Cli.CommandOption(\"--review-only\")]");
            sb.AppendLine("            [System.ComponentModel.Description(\"Return an exact mutation plan without changing Excel\")]");
            sb.AppendLine("            public bool ReviewOnly { get; init; }");
            sb.AppendLine();

            sb.AppendLine("            [Spectre.Console.Cli.CommandOption(\"--review-id <REVIEW_ID>\")]");
            sb.AppendLine("            [System.ComponentModel.Description(\"Bound review identifier returned by a prior review-only request\")]");
            sb.AppendLine("            public string? ReviewId { get; init; }");
            sb.AppendLine();

            sb.AppendLine("            [Spectre.Console.Cli.CommandOption(\"--checkpoint\")]");
            sb.AppendLine("            [System.ComponentModel.Description(\"Create a recoverable workbook checkpoint immediately before a mutation\")]");
            sb.AppendLine("            public bool Checkpoint { get; init; }");
            sb.AppendLine();

            sb.AppendLine("            [Spectre.Console.Cli.CommandOption(\"--idempotency-key <KEY>\")]");
            sb.AppendLine("            [System.ComponentModel.Description(\"Client-generated key for safely retrying a mutation execution\")]");
            sb.AppendLine("            public string? IdempotencyKey { get; init; }");
            sb.AppendLine();
        }
