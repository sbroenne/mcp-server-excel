<#
.SYNOPSIS
    Builds Copilot CLI plugins by COPYING validated plugin templates and updating versions.

.DESCRIPTION
    Phase 3 build script that preserves Phase 1/2 validated plugin implementations.

    COPY STRATEGY (not generate):
    1. Copy validated plugin structure from ../mcp-server-excel-plugins/plugins/
    2. Update version in plugin.json and version.txt
    3. Refresh skills content from source repo
    4. Refresh shared references from source repo
...
.PARAMETER PluginTemplateDir
    Validated plugin templates source. Default: ../mcp-server-excel-plugins/plugins/
...
This directory contains validated Phase 1/2 plugin implementations.
...
Write-Host "  ✓ excel-cli (CLI Skill)" -ForegroundColor Green
Write-Host ""
